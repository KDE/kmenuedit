...
void BasicTab::apply()
{
    if (_menuEntryInfo) {
        ...
        /* TODO: Should we leave the key with its old value (file dialog behaviour) or remove it (as below), or even set it to the same value?
        // Backwards compatible entry for old key, we can delete it as we
        // consider it as a fallback when reading data but don't update it
